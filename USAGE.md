...
Windget은 Windows 데스크탑 위에서 사용하는 WPF 위젯 앱입니다. 메모, 시스템 리소스, 사운드 믹서, 캘린더, Timer/Stopwatch, Quick Launcher를 투명한 데스크탑 캔버스 위에 배치할 수 있습니다.

## 설치

### Release ZIP으로 설치

1. GitHub Releases에서 `Windget-v0.1.0-win-x64.zip` 파일을 다운로드합니다.
2. 원하는 위치에 압축을 풉니다.
3. 압축을 푼 폴더의 `WindgetApp.exe`를 실행합니다.
...
`Start With Windows`는 현재 실행 중인 `WindgetApp.exe` 경로를 Windows 시작 프로그램에 등록합니다. 앱 폴더를 옮겼다면 Windget을 다시 실행한 뒤 `Start With Windows`를 껐다가 다시 켜 주세요.
...
- 위젯을 움직이거나 크기를 조절할 때 다른 위젯의 좌/중앙/우, 상/중앙/하 기준선에 가까워지면 정렬 가이드가 표시됩니다.
...
- `Start With Windows`: Windows 로그인 시 자동 실행 등록/해제
- 위젯 타일: 원하는 위젯 표시/숨김
- `Auto`: 해상도 기반 자동 배치
- `Save`: 현재 레이아웃 저장
...
- 설정에서 `Icon Only`를 켜면 바로가기를 아이콘 타일만으로 표시할 수 있습니다.

## 시스템 트레이

앱을 숨기면 시스템 트레이에 남습니다. 아이콘이 바로 보이지 않으면 Windows 작업 표시줄 오른쪽의 위쪽 화살표 안에서 찾을 수 있습니다.

- 트레이 아이콘 왼쪽 클릭: Control Center 표시/숨김
- 트레이 아이콘 더블 클릭: 위젯 다시 열기
- 트레이 메뉴 `Open Widgets`: 위젯 다시 열기
- 트레이 메뉴 `Control Center`: Control Center 표시/숨김
- 트레이 메뉴 `Exit`: 종료
